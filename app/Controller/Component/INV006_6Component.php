...
class INV006_6Component extends Component {
    ...
    public function chart($start, $end) {
        ...
        //Get data for Indicator INV006
        for ($i = 12; $i <= 23; $i++) {
            array_push($d12_d23, $this->Data->findByNombre('D' . $i));
        }
        ...
        for ($year = $start; $year <= $end; $year++) {
            ...
            foreach ($universities as $university) {
                ...
                foreach ($d12_d23 as $data_d_) {
                    ...
                    if (!empty($uy_d_)) {
                        array_push($uy_d12_d23, $uy_d_['UniversityYearData']['valor']);
                    } else {
                        array_push($uy_d12_d23, 0);
                    }
                }
                ...
                if (!empty($uy_d12_d23)) {
                    foreach ($disciplines as $index => $discipline) {
                        $disciplines[$index%6] = $disciplines[$index%6] + $uy_d12_d23[$index];
                    }
                }
            }
        ...
        FirePHP::getInstance(true)->log($ticks);
