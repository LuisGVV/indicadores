...
class INV006_6Component extends Component {
    ...
    public function chart($start, $end) {
        ...
        for ($i = 6; $i <= 11; $i++) {
            array_push($d6_d11, $this->Data->findByNombre('D' . $i));
        }
        ...
        for ($year = $start; $year <= $end; $year++) {
            ...
            foreach ($universities as $university) {
                ...
                foreach ($d6_d11 as $data_d_) {
                    ...
                    if (!empty($uy_d_)) {
                        array_push($uy_d6_d11, $uy_d_['UniversityYearData']['valor']);
                    } else {
                        array_push($uy_d6_d11, 0);
                    }
                }
                ...
                if (!empty($uy_d6_d11)) {
                    foreach ($disciplines as $index => $discipline) {
                        $disciplines[$index%6] = $disciplines[$index%6] + $uy_d6_d11[$index];
                    }
                }
            }
